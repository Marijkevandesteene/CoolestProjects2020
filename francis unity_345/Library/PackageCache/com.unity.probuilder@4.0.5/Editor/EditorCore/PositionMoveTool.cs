            ...
            if (EditorGUI.EndChangeCheck() && delta.sqrMagnitude > k_MinTranslateDeltaSqrMagnitude)
            {
                ...
                else if (progridsSnapEnabled)
                {
                    if (snapAxisConstraint)
                    {
                        ...
                        {
                            m_HandlePosition = UnityEngine.ProBuilder.Snapping.SnapValueOnRay(
                        ...
                        {
                            m_HandlePosition = UnityEngine.ProBuilder.Snapping.SnapValue(m_HandlePosition, progridsSnapValue);
                        }
                    }
                    else
                    {
                    ...
                    foreach (var index in group.indices)
                    {
                        ...
                        if (progridsSnapEnabled && !m_SnapAsGroup)
                        {
                            if (snapAxisConstraint && m_ActiveAxesWorld.active == 1)
                            {
                                ...
                                var snap = UnityEngine.ProBuilder.Snapping.SnapValueOnRay(
                            ...
                            else
                            {
                                ...
                                var snap = UnityEngine.ProBuilder.Snapping.SnapValue(wp, Vector3.one * progridsSnapValue);
                                positions[index] = worldToLocal.MultiplyPoint3x4(snap);
                            }
                        }
                        else
